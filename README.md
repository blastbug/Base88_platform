...
- [x] ロール/権限・初期管理者 seeder（＋デモデータ）
- [x] Filament 管理パネル雛形（platform_admin 制限）
- [x] **認証API（ログイン/ログアウト/me、Sanctumトークン）**
- [x] **案件 CRUD・検索・応募・成約(依頼先決定)・完了/キャンセル API**
- [x] **顧客情報の開示制御（成約会社のみ・API層Policy／セキュリティ検証済み）**
- [x] **会社単位データ分離・二重応募防止・自社案件応募禁止**
- [x] **フロント各画面**：ログイン／ホーム／案件検索／案件詳細・応募／案件掲載／自社案件／応募履歴／マイページ（レスポンシブ対応）
- [x] デザインシステム（ブランドUI・共通コンポーネント）
- [ ] Filament 管理画面の作り込み（会社承認・案件/応募/成約管理・操作ログ表示）
- [ ] パスワード再設定フロー・メール通知（応募/成約/登録）
- [ ] 締切超過の自動募集終了バッチ（スケジューラ）
- [ ] 添付ファイル（画像・PDF）アップロードUI

### デモ用ログイン（seeder投入）

| 種別 | メール | パスワード | 用途 |
|---|---|---|---|
| 加盟会社（発注/受注） | `demo1@base88.local` | `password` | 会社ユーザーとして全画面を確認 |
| 別の加盟会社 | `demo2@base88.local` | `password` | 応募・成約の相手側 |
| BASE88管理者 | `admin@base88.local` | `password` | `/admin`（Filament） |

> フロント（`http://127.0.0.1:3010` 等）から API（`http://127.0.0.1:8000`）を呼ぶ構成。
> `frontend/.env.local` の `NEXT_PUBLIC_API_URL` は **127.0.0.1** 推奨（localhost の IPv6 解決回避）。
